...
def example(num_samples=10, num_features=2, grid_size=20, filename="svm.pdf"):
    #randomly generate samples and labels
    samples = np.matrix(np.random.normal(size=num_samples * num_features)
                        .reshape(num_samples, num_features))
    
    labels = 2 * (samples.sum(axis=1) > 0) - 1.0
    # create a svm with a linear kernel
    #   other choices would be: .gaussian(sigma); radial_basis(gamma=10); 
    trainer = svmpy.SVMTrainer(svmpy.Kernel.linear(), 0.1)
    # learn the model from training samples/labels
